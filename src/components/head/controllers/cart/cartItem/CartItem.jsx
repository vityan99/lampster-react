...
function CartItem({ id, img, title, price, count, deleteProductHandler }) {
  const clickDeleteHandler = (id) => deleteProductHandler(id);

  return (
    <div className={style["mini-product"]}>
      <div className={style["mini-product__img-wrapper"]}>
        <img className={style["mini-product__img"]} src={require(`../../../../../assets/img/${img}`)} alt="product" />
      </div>
      ...
      <button className={`${style.btn} ${style["btn--delete"]}`}>
        <FontAwesomeIcon icon={faXmark} style={{ color: "#f56a43" }} onClick={() => clickDeleteHandler(id)} />
      </button>
    </div>
  );
}
